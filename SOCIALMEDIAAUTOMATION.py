...
app = FastAPI(title="SOCIALMEDIAAUTOMATION", version="2.0.1")
...
class PublishPayload(BaseModel):
    ...
    media_id: Optional[str] = Field(
        default=None,
        description="Optional media identifier from Meta/Make for traceability.",
    )
...
async def root() -> Dict[str, Any]:
    return {
        ...
        "version": "2.0.1",
        "configured_ids": {
            "default_post_id": os.getenv("DEFAULT_POST_ID"),
            "default_media_id": os.getenv("DEFAULT_MEDIA_ID"),
...
async def handle_webhook(request: Request, x_make_secret: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    ...
    if body.get("event_type") == "publish_post":
